...
const transactionHistoryReducerEpic = (action$:any) => action$.pipe(
    ofType('UPDATE_TRANSACTIONS'),
    flatMap(() => transactionHistoryApi.apiTransactionEmployeeEmployeeIdGet({employeeId:"dfghj"})),
    map(transactions => ({ type: 'UPDATE_TRANSACTIONS_SUCCESS', payload: transactions }))
);
